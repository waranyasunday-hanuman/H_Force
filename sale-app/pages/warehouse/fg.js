// pages/warehouse/fg.js – คลัง Finish Goods (FG) Approval Dashboard
...
const OP_CFG = {
    issue:    { label: "ขอเบิก",  icon: "📤", color: "bg-indigo-50 text-indigo-600" },
...
export default function FGWarehousePage() {
    ...
    const fetchRequests = async () => {
        setLoading(true);
        try {
            const res = await fetch("/api/warehouse/issue/list?type=FINISH_GOODS");
    ...
    const handleApprove = async (req) => {
        ...
        try {
            const res = await fetch("/api/warehouse/issue/approve", {
                ...
                body: JSON.stringify({
                    request_id: req.id,
                    approved_by: "เจ้าหน้าที่คลัง FG",
            ...
            <div suppressHydrationWarning className="max-w-2xl mx-auto pb-24 animate-fade-up">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div className="flex items-center gap-3 flex-1">
                        <div className="w-10 h-10 rounded-2xl bg-indigo-500 flex items-center justify-center text-xl shadow-lg shadow-indigo-200">
                            📦
                        </div>
                        <div>
                            <h2 className="text-xl font-black text-slate-900 tracking-tight">คลัง Finish Goods</h2>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">FG Warehouse Approval</p>
                        </div>
                    </div>
                ...
                <div className="mb-4">
                    <input type="text" placeholder="ค้นหาเลขที่คำขอ / ผู้ขอ..." value={search}
                        ...
                        className="w-full px-4 py-2.5 bg-white rounded-xl border border-slate-200 text-sm font-bold focus:ring-2 focus:ring-indigo-400 outline-none"/>
                </div>
